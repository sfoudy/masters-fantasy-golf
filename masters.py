            ...
            for competition in event.get('competitions', []):
                for player in competition.get('competitors', []):
                    try:
                        ...
                        actual_score = int(score) if score else 0
                        
                        # Cut detection
                        status = player.get('status', {}).get('type', '').upper()
                        penalty = 10 if status == 'CUT' else 0
...
def main():
    ...
    for team, golfers in st.session_state.teams.items():
        ...
        leaderboard.append({
            ...
            "Golfers": ", ".join([f"{proper_case(g)} ({live_scores[normalize_name(g)]['actual']:+})" for g in valid_golfers])
        })
