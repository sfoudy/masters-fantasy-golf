...
# Correct score extraction using relative-to-par values
            ...
            for competition in event.get('competitions', []):
                for player in competition.get('competitors', []):
                    try:
                        ...
                        # Get score relative to par (tournament total)
                        score = player.get('totalToPar', player.get('scoreToPar', 'E'))
...
def main():
    ...
    # Load scores with accurate relative-to-par values
    live_scores = get_masters_scores() or {
        normalize_name("Scottie Scheffler"): -8,
